classdef husion < handle
    properties
        posteria
        weightMatrix
        subclassToClassMapping
        XterrainMesh
        YterrainMesh
    end
    
    methods
        function addhusion(obj)                                       %add value for blank Landmark
            ...
            %% Add library paths
            libPath = 'G:\My Drive\เรียน\year 3\ฝึกงาน\ASL\Git\Husion-Simulator\MatlabLib'; % path to MatlabLib folder; % param
            ...
            obj.subclassToClassMapping=subclassToClassMapping;
            %%
            ...
            D = input('value of D :'); % 1="Near" 2="Far" or 3="At"   % param
            ...
            [XterrainMesh, YterrainMesh] = meshgrid(xGrid, yGrid);
             obj.XterrainMesh=XterrainMesh;
             obj.YterrainMesh=YterrainMesh;
        ...
        function addD(obj)
            ...
            figure(2) %%%%figure(2)
            surf(obj.XterrainMesh,obj.YterrainMesh, exp(lnpXgivenD'),'EdgeColor','none')
            view(2)
             title(['Posterior Given Human Measurement p(x|{\zeta^{ HUMAN}}); D = ' num2str(D) ', x_{l} = [' num2str(xl_in(1)) ' ' num2str(xl_in(2)) ']^T '],'fontsize',15)
            xlabel('x_0 (m)','fontsize',15)
            ylabel('x_1 (m)','fontsize',15)
